...
- **Trailing dot** (`onTrailMove`/`tick`): a small dot eases toward the cursor at a fixed lerp factor (0.07/frame) via its own `requestAnimationFrame` loop, and flattens into a horizontal dash (`.dot--link`) while hovering any `<a>`.
...
- Fonts: `IBM Plex Mono` (Google Fonts) for all-caps/technical bits (logo, labels, URLs, readout), system sans (`Helvetica Neue`/Helvetica/Arial) for everything else.
